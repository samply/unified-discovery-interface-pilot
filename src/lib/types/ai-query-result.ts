...
export class AiQueryResult {
    private constructor(
        ...
        public readonly age_at_diagnosis: Record<string, any>,
        public readonly date_of_diagnosis: string,
        public readonly patient_age: Record<string, any>,
        public readonly sample_type: string[],
        public readonly sampling_date: string,
    ...
    public getGender(): 'male' | 'female' | 'unknown' | 'other' | null {
        if (!this.validateField(this, 'gender', 'string')) return null;
    ...
    public getDiagnosis(): string[] | null {
        if (!this.validateField(this, 'diagnosis', 'string[]')) return null;
    ...
    public getSampleType(): string[] | null {
        if (!this.validateField(this, 'sample_type', 'string[]')) return null;
    ...
    private parseDateRange(fieldName: string): { lower: Date | null; upper: Date | null } | {} | null {
        const range = (this as any)[fieldName];
     ...
     * @returns {{ lower: Date | null; upper: Date | null } | null | {}}
     */
    public getDateOfDiagnosis(): { lower: Date | null; upper: Date | null } | null | {} {
        ...
        if (noDiagnosis && datesAreEqual) {
            console.warn('Diagnosis dates match sampling dates and no diagnosis exists. Returning empty object.');
            return {};
        }
     ...
     */
    private parseAgeRange(fieldName: string): { lower: number | null; upper: number | null } | {} | null {
        if (!this.validateField(this, fieldName, 'object')) return null;
     ...
     */
    public getPatientAge(): { lower: number | null; upper: number | null } | {} | null {
    ...
    public getAgeAtDiagnosis(): { lower: number | null; upper: number | null } | {} | null {
        const diagnosis = this.getDiagnosis();
    ...
    private normalizeStorageTemperature = (input: string): string => {
        ...
        if (/liquid\s?nitrogen|cryopreserv|(-1\d{2,}\s?°?c)/.test(value)) return 'temperatureLN';
        ...
        console.warn(`Unrecognized temperature input: "${input}" – defaulting to temperatureOther.`);
        return 'temperatureOther';
    };

    public getSampleStorageTemperature(): string[] | null {
        if (!this.validateField(this, 'sample_storage_temperature', 'string[]')) return null;
